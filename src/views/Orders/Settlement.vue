    ...
    <div class="view-body" v-if="orderInfo">
      <div>
        ...
        <section class="cart-address">
          <p class="title">订单配送至
            <span class="sddress-tag" v-if="userInfo && userInfo.tag">{{userInfo.tag}}</span>
          </p>
          <p class="address-detail">
            <span v-if="userInfo"
                  @click="$router.push('/myAddress')">{{userInfo.address}}{{userInfo.bottom}}</span>
            <span v-else>
        ...
        <section class="checkout-section">
          <cart-item @click="showTableware=true" title="餐具份数" :subHead="remarkInfo.tableware || '未选择'" />
          <cart-item @click="$router.push('/remark')" title="订单备注" :subHead="remarkInfo.remark || '口味 偏好'" />
...
export default {
  ...
  methods: {
    ...
    getData() {
      this.$axios('/api/user/user_info/5f9f837d99b90300175587d0')
      .then( res => {
...
<style scoped>
  ...
  .view-body {
    ...
    background-image: linear-gradient(
      ...
      linear-gradient(270deg, rgb(54, 138, 248), rgb(54, 138, 248));
    display: flex;
...
.action-bar button {
  flex: 2;
  background-color: rgb(57, 235, 102);
  outline: none;
  border: none;
  height: 100%;
  color: #fff;
